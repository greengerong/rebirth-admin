...
describe('AppComponent', () => {
    ...
    TestBedUtils.configureTestingModule({
      ...
      providers: [
        ...
        },
        {
          ...
          useValue: jasmine.createSpyObj('authorizationService', ['getCurrentUser', 'setStorageType'])
        }
      ]
    }).compileComponents();
  ...
  it(`should setup http request interceptors`, async(inject([RebirthHttpProvider, LoadingService, AuthorizationService],
    (rebirthHttpProvider: RebirthHttpProvider, loadingService: LoadingService, authorizationService: AuthorizationService) => {
      const request = new HttpRequest<any>('GET', 'url');
      rebirthHttpProvider.getInterceptors()
  ...
  it(`should setup Authorization token`, async(inject([RebirthHttpProvider, LoadingService, AuthorizationService],
    (rebirthHttpProvider: RebirthHttpProvider, loadingService: LoadingService, authorizationService: AuthorizationService) => {
      const request = new HttpRequest<any>('GET', 'url');
      const currentUser = { token: 'token' };
      (authorizationService.getCurrentUser as any).and.returnValue(currentUser);
      const result = rebirthHttpProvider.getInterceptors()
        .filter(item => item.request)
        .reduce((req, item) => item.request(req) || req, request);
  ...
  it(`should setup http response interceptors`, async(inject([RebirthHttpProvider, LoadingService],
    (rebirthHttpProvider: RebirthHttpProvider, loadingService: LoadingService) => {
  ...
  it(`should redirect to login when auth error`, async(inject([RebirthHttpProvider, Router],
  ...
  it(`should not redirect to login when not auth error`, async(inject([RebirthHttpProvider, Router],
    (rebirthHttpProvider: RebirthHttpProvider, router: Router) => {
